...
namespace octsock5 {
    ...
    public class octsock5_cl:IDisposable {
        ...
        public octsock5_cl(string portId, bool isServer) {
            // name of the "Windows named pipe". Agreed between different implementations
            string pipename = "octsock5_"+portId;
            if(isServer) {
        ...
        /// <summary>
        /// Server only: blocks until connection is made.
        /// A server should call the constructor, then signal the remote end that the server is up (e.g. via STDOUT), then call accept().
        /// This solves the problem that some time passes between starting the server, and the availability of the connection.
        /// </summary>
        public void accept() {
            if(server == null)
                throw new Exception("accept() may only be called in server mode");
